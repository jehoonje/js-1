...
document.querySelector(".todo-list").addEventListener("click", (e) => {
  ...
  if (e.target.matches(".remove span")) {
    ...
    const $undoSpan = $targetLi.querySelector(".modify span"); // 수정 버튼 선택
    $undoSpan.classList.remove("lnr-undo"); // 수정 버튼의 클래스 변경
    $undoSpan.classList.add("lnr-checkmark-circle");
    ...
    $undoSpan.addEventListener("click", () => {
      ...
      const newText = $input.value.trim(); // 입력된 새 텍스트 값을 가져오고 양쪽 공백 제거
      // console.log(newText);
      if (newText === "") return; // 입력된 텍스트가 없으면 함수 종료
      $input.replaceWith($textSpan); // input 요소를 span 요소로 대체
      $textSpan.textContent = newText; // span 요소의 텍스트 값을 새로 입력된 값으로 변경
      console.log(newText);
      $undoSpan.classList.remove("lnr-checkmark-circle"); // 수정 완료 버튼의 클래스 변경
      $undoSpan.classList.add("lnr-undo");
      $input.remove();
      $undoSpan.remove();
